      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        {project.flags.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-500 mb-4">No flags yet. Create your first feature flag!</p>
            <button
              onClick={() => setIsModalOpen(true)}
              ...
              <thead>
                <tr className="border-b border-gray-200">
                  ...
                  {project.environments.map((env) => (
                    <th key={env.id} className="text-center py-3 px-4 font-semibold text-gray-700 capitalize">
                      {env.name}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {project.flags.map((flag) => (
                  <tr key={flag.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    {project.environments.map((env) => {
                      const state = flag.states.find((s) => s.environment.id === env.id)
                      return (
                        <td key={env.id} className="py-4 px-4 text-center">
                          <button
                            onClick={() => toggleFlag(flag.id, env.id, !state?.enabled)}
